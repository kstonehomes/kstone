...
const QuickPossessionCard = ({ item }: QuickPossessionCardProps) => {
  console.log(item)
  const colorStatus = {
  ...
  // const showPrice = (typeof item.newPrice === "number") && (item.status === "ready" || item.status === "pending");
  const hasSavings = (typeof item.oldPrice === "number") && (typeof item.newPrice === "number") && item.oldPrice > item.newPrice;

  return (
    <Link href={`/quick-possessions/${item.slug}`} className="group relative bg-primary overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-700 hover:-translate-y-3 transform "> 
      {/* Image Container */}
      <div className="relative w-full aspect-[5/4] overflow-hidden">
        ...
        {item.status === "ready"  && (
          <div className="absolute bottom-0 right-0 bg-white backdrop-blur-sm p-3 shadow-xl border border-white transform group-hover:scale-105 transition-all duration-300">
            {hasSavings && (
              <div className="text-xs font-base text-red-500 line-through text-center">
                ${item.oldPrice?.toLocaleString() ?? "-"}
              </div>
            )}
            <div className="text-lg font-display font-bold text-green-500 text-center">
              {typeof item.newPrice === "number" ? `$${item.newPrice.toLocaleString()}` : "Contact for price"}
            </div>
            {hasSavings && (
              <div className="text-xs font-display font-semibold text-primary text-center mt-1">
                Save ${typeof item.oldPrice === "number" && typeof item.newPrice === "number" ? (item.oldPrice - item.newPrice).toLocaleString() : "-"}
              </div>
            )}
          </div>
        )}
...
export default QuickPossessionCard;