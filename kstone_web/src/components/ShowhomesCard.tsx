...
import { FaBath, FaBed, FaRulerCombined, FaArrowRight, FaMapLocation } from "react-icons/fa6";
import {  ShowHomeProps } from "@/types/propsInterfaces";
import { FaMapMarkerAlt } from "react-icons/fa";
interface QuickPossessionCardProps {
  item: ShowHomeProps;
}



// function getDisplayStatus(item:ShowHomeProps) {
//   if (!item.status) return null;
//   if (item.status === "ready" && item.readyStatus) {
//     return `${item.status} - ${item.readyStatus}`;
//   }
//   if (item.status === "available" && item.availableStatus) {
//     return `${item.status} - ${item.availableStatus}`;
//   }
//   return item.status;
// }

const ShowhomesCard = ({ item }: QuickPossessionCardProps) => {
  ...
  return (
    <Link href={`/showhomes/${item.slug}`} className="group relative bg-primary h-[700px]_ overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-700 hover:-translate-y-3 transform ">
      {/* Image Container */}
      <div className="relative w-full aspect-[5/4] overflow-hidden">
        ...
        {/* Status Badge */}
        {item.status && (
          <div className={`absolute top-0 left-0 px-7 py-3 text-lg font-display font-medium text-white shadow-lg backdrop-blur-sm transform group-hover:scale-105 transition-all duration-300 ${colorStatus[item.status as keyof typeof colorStatus]}`}>
            {item.status && (
              <span className="capitalize font-medium">{item.status?.toUpperCase()}</span> 
            ) }
          </div>
      
          // <div className={`absolute top-0 left-0 px-7 py-2 text-lg font-display font-medium text-white shadow-lg backdrop-blur-sm transform group-hover:scale-105 transition-all duration-300 capitalize ${colorStatus[item.status as keyof typeof colorStatus]}`}>
      ...
      <div className="p-6 relative bg-white h-full">
        ...
        <div className="space-y-1">
          ...
          <p className="font-base text-gray-600 text-sm">
            <span className="font-medium hidden">Type:</span> {item.houseType}
          </p>
