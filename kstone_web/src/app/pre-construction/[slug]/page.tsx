...
import { FloorPlan, PreConstructionDetail } from "@/types/propsInterfaces";
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const propertyData = await client.fetch<PreConstructionDetail | null>(
    `*[_type == "preConstruction" && slug.current == $slug][0]{
      houseModel,
      "city": city->{name},
  ...
  return {
    title: `${propertyData.houseModel} | ${propertyData.community.name}`,
    description:
      propertyData.shortDescription ??
      `Explore the ${propertyData.houseModel} in ${propertyData.community.name}.`,
    openGraph: {
      images: [propertyData.featuredImage],
    },
  };
}

export default async function PreConstructionPage({ params }: Params) {
  const { slug } = await params;
        ...
        <div className="container">
          <div className="bg-white shadow-2xl- border border-gray-100 overflow-hidden">
            ...
            <div className="relative w-full aspect-[16/8] overflow-hidden">
              <Image
                src={propertyData.featuredImage}
                alt={propertyData.houseModel}
                fill
        ...
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {propertyData.floorPlans.map((floor: FloorPlan, i: number) => (
