...
export const floorPlans = defineType({
  name: 'floorPlans',
  title: 'Floor Plans', 
  type: 'document',
  fields: [
    defineField({
      ...
      validation: (Rule) =>
        Rule.required()
          .min(4)
          .max(100)
          .error('Project name is required and must be between 4 and 100 characters.'),
    ...
    defineField({
      ...
      type: 'slug',
      options: {
        source: 'floorPlanName',
        maxLength: 96,
      },
      validation: (Rule) => Rule.required().error('Slug is required and must be unique.'),
      description: 'A unique identifier for the Project Layout, used in URLs.',
    }),
    defineField({
      name: 'floorPlanModel', 
      title: 'Floor Plan Model',
      type: 'string',
      description: 'Select the type of the project (e.g., Front-Drive Home, Landed Home).',
      validation: (Rule) =>
        Rule.required(),
      options: {
        list: [
